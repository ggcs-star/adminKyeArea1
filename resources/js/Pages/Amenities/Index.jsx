...
const AmenitiesPage = ({ auth, projects, allProjects }) => {
    ...
    const handleDelete = (projId) => {
        if (confirm("Are you sure you want to delete amenities?")) {
            Inertia.delete(`/amenities/${projId}/delete`);
        }
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                        <div className="mb-4 sm:mb-0">
                            ...
                            <p className="text-gray-600 mt-2">Manage amenities and facilities for all projects</p>
                        </div>
                                    ...
                                    {projects.data?.length > 0 ? (
                                        projects.data.map((proj, idx) => (
                                            <tr key={proj.id} className="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${proj.amenities && Object.keys(proj.amenities).length > 0
                                                            ? 'bg-green-100 text-green-800'
                                                            : 'bg-yellow-100 text-yellow-800'
                                                        }`}>
                                                        <svg className={`w-3 h-3 mr-1 ${proj.amenities && Object.keys(proj.amenities).length > 0 ? 'text-green-500' : 'text-yellow-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={proj.amenities && Object.keys(proj.amenities).length > 0 ? "M5 13l4 4L19 7" : "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"} />
                                                        </svg>
                                                        {proj.amenities && Object.keys(proj.amenities).length > 0 ? "Configured" : "Not Configured"}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center space-x-2">
                                                        <button
                                                            onClick={() => Inertia.get(`/amenities/${proj.id}/show`)}
                                        ...
                                        <tr>
                                            <td colSpan="4" className="px-6 py-12 text-center">
                                                <div className="flex flex-col items-center justify-center text-gray-400">
                                                    ...
                                                    <p className="text-lg font-medium text-gray-500">No amenities found</p>
                                                    <p className="text-sm text-gray-400 mt-1">Get started by creating your first amenities configuration</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
