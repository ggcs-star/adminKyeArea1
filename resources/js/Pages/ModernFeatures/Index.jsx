...
import ModernFeaturesForm from "@/Components/ModernFeaturesForm";
...
const Index = ({ auth, projects, allProjects }) => {
    ...
    const handleDelete = (id) => {
        if (confirm("Are you sure you want to delete?")) {
            Inertia.delete(`/modern-features/${id}/delete`);
        }
    };

    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Modern Features" />
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                        <div className="mb-4 sm:mb-0">
                            ...
                            <p className="text-gray-600 mt-2">Manage cutting-edge features and smart technologies for projects</p>
                        </div>

                        <button
                            onClick={() => { setSelectedProject(null); setShowForm(true); }}
                            className="flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-xl shadow-lg hover:from-blue-700 hover:to-cyan-700 transition-all duration-200 transform hover:-translate-y-0.5"
                        >
                                            ...
                                            <tr key={proj.id} className="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center">
                                                        <div className="bg-gradient-to-br from-blue-100 to-cyan-100 p-2 rounded-lg mr-3">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${proj.modern_features && Object.keys(proj.modern_features).length > 0
                                                            ? 'bg-green-100 text-green-800'
                                                            : 'bg-yellow-100 text-yellow-800'
                                                        }`}>
                                                        <svg className={`w-3 h-3 mr-1 ${proj.modern_features && Object.keys(proj.modern_features).length > 0 ? 'text-green-500' : 'text-yellow-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={proj.modern_features && Object.keys(proj.modern_features).length > 0 ? "M5 13l4 4L19 7" : "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"} />
                                                        </svg>
                                                        {proj.modern_features && Object.keys(proj.modern_features).length > 0 ? "Configured" : "Not Configured"}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center space-x-2">
                                                        <Link
                                                            href={`/modern-features/${proj.id}/view`}
                                        ...
                                        <tr>
                                            <td colSpan="4" className="px-6 py-12 text-center">
                                                <div className="flex flex-col items-center justify-center text-gray-400">
                                                    ...
                                                    <p className="text-lg font-medium text-gray-500">No modern features found</p>
                                                    <p className="text-sm text-gray-400 mt-1">Get started by adding modern features to your projects</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
